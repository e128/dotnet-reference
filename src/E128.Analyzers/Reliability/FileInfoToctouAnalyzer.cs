...
public sealed class FileInfoToctouAnalyzer : DiagnosticAnalyzer
{
    ...
    private static readonly DiagnosticDescriptor Rule = new(
        id: DiagnosticId,
        title: "FileInfo.Exists TOCTOU race condition",
        messageFormat: "File read on '{0}' follows FileInfo.Exists check without try/catch guard — another process may delete the file between the check and the read",
        category: "Reliability",
        defaultSeverity: DiagnosticSeverity.Warning,
        isEnabledByDefault: true,
        description: "Checking FileInfo.Exists before reading a file creates a TOCTOU race. Guard the read with try/catch(IOException) instead of relying on the existence check.");
    ...
    private static bool IsQualifiedFileAccess(ExpressionSyntax expression) =>
        expression is MemberAccessExpressionSyntax { Name.Identifier.ValueText: "File" };
